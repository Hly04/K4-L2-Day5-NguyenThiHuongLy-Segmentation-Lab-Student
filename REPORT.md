...
- Công cụ đã dùng: CVAT Polygon và gợi ý tự động; export COCO 1.0 và Segmentation mask 1.1; công cụ kiểm cấu trúc ZIP của repo; scorer cục bộ cho ba tier
...
QC cấu trúc ngày 17/09/2026: đủ 9 ZIP, không có ZIP tên lạ. Các ZIP semantic có đủ PNG trong `SegmentationClass/` và một `labelmap.txt`; các ZIP instance/panoptic có một COCO JSON đọc được trong `annotations/`. Medium có 3 ảnh/74 annotation; Hard có 2 ảnh/61 annotation và đủ năm lớp stuff. Không phát hiện lỗi tên ảnh, tên lớp hoặc file rỗng qua cấu trúc export.
